...
class OpenAIServing:
    ...
    async def create_chat_completion(
        ...
        try:
            ...
            return self.create_error_response("test")
    ...
    async def chat_completion_stream_generator(
        ...
        if request.echo:
            ...
            if (
                ...
                and request.messages[-1].role == role
            ):
        ...
        async for res in result_generator:
            ...
            for output in res.outputs:
                ...
                if (
                    self.openai_tools_prompter is not None
                    and request.tools is not None
                ...
                if output.text.startswith(OpenAIToolsPrompter.func_call_token_pre()):
                    if output.finish_reason == "stop":  # Send tools call
                        ...
                        if not current_capture.calls_validation(tools_list=tools_list):
                            ...
                            for call_id in range(calls_count):
                                tool_calls.append(
                                    current_capture.to_ToolCallsMessage(func_id=call_id)
                                )
                            message = ChatMessage(
    ...
    def create_error_response(
        ...
        return ErrorResponse(message=message, type=err_type, code=str(status_code.value))
